<?php
	...
	*	sf_get_typekit_id()
	*	sf_get_typekit_json_response()
	*	sf_get_typekit_kit()
	*	sf_create_typekit_font_family_array()
	*	sf_add_typkit_to_redux_custom_fonts()
	*	sf_typekit_enqueue_script()
	*	sf_typekit_inline_script()
	*	sf_typkit_admin_css()
	*
	*
	*	OVERRIDES
...
//			$logo_alt = get_bloginfo( 'name' );
    ...
    /* GET TYPKIT KIT ID
    ================================================== */
    if ( ! function_exists( 'sf_get_typekit_id' ) ) {
        function sf_get_typekit_id() {
            ...
            if ( isset( $sf_atelier_options['typekit_id'] ) ) {
                $typekit_id = $sf_atelier_options['typekit_id'];
            }
    ...
    ================================================== */
    if ( ! function_exists( 'sf_get_typekit_json_response' ) ) {
        function sf_get_typekit_json_response() {
            
            $kits = $typekit_id = '';
            $typekit_id = sf_get_typekit_id();
    ...
    ================================================== */
    if ( ! function_exists( 'sf_get_typekit_kit' ) ) {
        function sf_get_typekit_kit() {
            
            $out = $kits = '';
            $kits = sf_get_typekit_json_response();  
            ...
            if ( ! empty( $kits ) && is_object( $kits ) ) {
                ...
                $out .='<thead>';
                    $out .='<tr>';
                        $out .='<th>' . esc_html__( 'Font', 'swiftideas' ) . '</th>';
                        $out .='<th>' . esc_html__( 'Font Family CSS Value', 'swiftideas' ) . '</th>';
                        $out .='<th>' . esc_html__( 'Variations/Weights', 'swiftideas' ) . '</th>';
                        $out .='<th>' . esc_html__( 'URL', 'swiftideas' ) . '</th>';
                    $out .='</tr>';
                $out .='</thead>';
                $out .='<tbody>';
                foreach ( $kits->kit->families as $font_family ) {
                    ...
                    $italic = esc_html__( 'Italic', 'swiftideas' );
                    ...
                    $out .= esc_html__( 'View on Typekit', 'swiftideas' );
                    $out .='</a></td></tr>';
                }
                $out .='</tbody>';
                $out .='</table>'; 
            ...
            $out .= esc_html__( 'No Kit Available. If you just added your ID, please try saving options and wait for refresh.', 'swiftideas' );
    ...
    ================================================== */
    if ( ! function_exists( 'sf_create_typekit_font_family_array' ) ) {
        function sf_create_typekit_font_family_array( ) {
            ...
            if ( ! empty( $kits ) && is_object( $kits ) ) {
                foreach ($kits->kit->families as $font_family ) {
                    $font_families[$font_family->css_stack] = $font_family->name;
                }
            }
    ...
    ================================================== */
    if ( ! function_exists( 'sf_add_typkit_to_redux_custom_fonts' ) ) {
        function sf_add_typkit_to_redux_custom_fonts( $custom_fonts ) {
            
            $font_families = $font_out = '';
            $font_families = sf_create_typekit_font_family_array();

            if ( is_array( $font_families ) && !empty( $font_families ) ) {
                $font_out = array( esc_html__( 'Typekit Fonts', 'swiftideas' ) => $font_families );
            }

            return $font_out;
        }
        add_filter( 'redux/sf_atelier_options/field/typography/custom_fonts', 'sf_add_typkit_to_redux_custom_fonts' );
    }

    /* ENQUEUE TYPKIT SCRIPT - FRONTEND & ADMIN REQUIRED
    ================================================== */
    if ( ! function_exists( 'sf_typekit_enqueue_script' ) ) {
        function sf_typekit_enqueue_script() {
         ...
         add_action( 'wp_enqueue_scripts', 'sf_typekit_enqueue_script' );
         add_action('admin_enqueue_scripts', 'sf_typekit_enqueue_script');
    }

    /* ENQUEUE TYPKIT INLINE SCRIPT - FRONTEND & ADMIN REQUIRED
    ================================================== */
    if ( ! function_exists( 'sf_typekit_inline_script' ) ) {
        function sf_typekit_inline_script() {
        ...
        add_action( 'wp_head', 'sf_typekit_inline_script' );
        add_action( 'admin_head', 'sf_typekit_inline_script' );
    }


	if ( ! function_exists( 'sf_typkit_admin_css' ) ) {
	    function sf_typkit_admin_css() { 
	    	?>
	    	<style type="text/css" media="screen">
	    	/* Typkit Theme Options  */
            .sf-typekit-kits {
                border-spacing: 0;
                width: 100%;
                clear: both;
	    ...
	    add_action( 'admin_head', 'sf_typkit_admin_css' );
	}
		...
		function sf_ajaxsearch() {
			...
			if ( function_exists('relevanssi_do_query') ) {
				...
				if ( $search_results->have_posts() ) {
			    	...
			    	$search_results_ouput .= '<div class="no-search-results" data-test="123">';
                ...
                <div class="top-author-info container clearfix">
                    ...
                    <div class="post-details">
                        <div class="author-name" itemprop="author" itemscope itemtype="http://schema.org/Person">
                        	<h5 class="vcard author"><?php echo sprintf( __( 'By <a href="%2$s" rel="author" class="fn"><span itemprop="name">%1$s</span></a>', 'swiftframework' ), $author_name, $author_url ); ?></h5>
                        </div>
                        <?php if ( !$remove_dates ) { ?>
