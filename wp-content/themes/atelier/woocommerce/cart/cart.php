	...
	<form class="woocommerce-cart-form" action="<?php echo esc_url( wc_get_cart_url() ); ?>" method="post">
		...
		<div class="row">
			...
			<div class="col-sm-8 cart-items-wrap">
				...
				<h3 class="bag-summary"><?php esc_html_e('Your selection', 'swiftframework');?> <span>(<?php echo esc_attr($cart_count); ?>)</span></h3>
						...
						foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
							...
							if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
								...
								<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
										...
										<?php
											...
											if (function_exists('wc_get_formatted_cart_item_data')) {
				               				...
				               				if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) )
												echo wp_kses_post( apply_filters( 'woocommerce_cart_item_backorder_notification', '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'swiftframework' ) . '</p>' ) );
											?>
									</td>
									...
									<td class="product-remove" data-title="<?php esc_attr_e( 'Remove', 'swiftframework' ); ?>">
										<?php
							...
							<td colspan="6" class="actions">
								...
								<?php if ( wc_coupons_enabled() ) { ?>
									<div class="coupon">
										...
										<label for="coupon_code"><?php esc_html_e( 'Coupon:', 'swiftframework' ); ?></label> <input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Coupon code', 'swiftframework' ); ?>" /> <button type="submit" class="button" name="apply_coupon" value="<?php esc_attr_e( 'Apply Coupon', 'swiftframework' ); ?>"><?php esc_attr_e( 'Apply coupon', 'swiftframework' ); ?></button>
								...
								<button type="submit" class="button" name="update_cart" value="<?php esc_attr_e( 'Update cart', 'swiftframework' ); ?>"><?php esc_html_e( 'Update cart', 'swiftframework' ); ?></button>
