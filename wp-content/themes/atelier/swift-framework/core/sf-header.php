        ...
        function sf_get_cart() {
            ...
            if ( sf_woocommerce_activated() ) {
                ...
                if ( $cart_count != "0" ) {
                    ...
                    $cart_output .= '<a class="sf-button standard sf-icon-reveal checkout-button" href="' . esc_url( wc_get_checkout_url() ) . '">'.$checkout_icon.'<span class="text">' . __( 'Checkout', 'swiftframework' ) . '</span></a>';
                ...
                foreach ( $wishlist as $values ) :
                    ...
                    if ( $count_limit < 3 ) {
                        ...
                        if ( $product_obj !== false && $product_obj->exists() ) :
                            ...
                            $wishlist_output .= $product_obj->get_price() ? $product_obj->get_price_html() : apply_filters( 'yith_free_text', __( 'Free!', 'swiftframework' ) );
        ...
        function sf_get_account( $aux = "" ) {
            ...
            if ( is_user_logged_in() ) {
                ...
                if ( function_exists('bp_loggedin_user_domain') ) {
                	$account_output .= '<li class="menu-item aux-bp-profile"><a href="' . bp_loggedin_user_domain() . '">' . __( "Profile", "swiftframework" ) . '</a>
                	</li>' . "\n";
                }
                if ( function_exists('bp_is_active') && bp_is_active('messages') ) {
                	$account_output .= '<li class="menu-item aux-bp-messages"><a href="' . bp_loggedin_user_domain() . 'messages/">' . sprintf( __( "Messages (%d)", "swiftframework" ), messages_get_unread_count() ) . '</a>
                	</li>' . "\n";
                }
                if ( function_exists('bp_loggedin_user_domain') ) {
                	$user_id = bp_loggedin_user_id();
                	$notificaiton_count = bp_notifications_get_unread_notification_count( bp_loggedin_user_id() ); 	
					$account_output .= '<li class="menu-item aux-bp-notifications"><a href="' . bp_loggedin_user_domain() . 'notifications/">' . sprintf( __( "Notifications (%d)", "swiftframework" ), $notificaiton_count ) . '</a>
					</li>' . "\n";
					
                	$account_output .= '<li class="menu-item aux-bp-settings"><a href="' . bp_loggedin_user_domain() . 'settings/">' . __( "Settings", "swiftframework" ) . '</a>
                	</li>' . "\n";
                }
    ...
    if ( ! function_exists( 'sf_ajaxsearch' ) ) {
        function sf_ajaxsearch() {
            ...
            if ( function_exists('relevanssi_do_query') ) {
                $search_results = relevanssi_do_query($search_results);
            }
